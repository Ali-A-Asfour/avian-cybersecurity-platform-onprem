...
// Function to get mock database instance only when needed
function getMockDbInstance(): MockDatabase {
  if (!global.__mockDb) {
...
// Export a proxy that only creates the instance when accessed
export const mockDb = new Proxy({} as MockDatabase, {
  get(target, prop) {
    // Only create the instance if we're in the right environment
    if (process.env.NODE_ENV === 'development' && process.env.BYPASS_AUTH === 'true') {
      const instance = getMockDbInstance();
      return instance[prop as keyof MockDatabase];
    }
    // If not in the right environment, throw an error
    throw new Error('MockDatabase is only available in development mode with BYPASS_AUTH=true');
  }
});