        ...
        const fetchCounts = async () => {
            try {
                ...
                if (userRole === UserRole.USER) {
                    ...
                    const response = await fetch('/api/tickets/user?limit=1');
                    if (response.ok) {
                    ...
                    const response = await fetch('/api/help-desk/queue/unassigned?limit=1');
                    if (response.ok) {
                        const data = await response.json();
                ...
                if (userRole === UserRole.SECURITY_ANALYST || userRole === UserRole.SUPER_ADMIN) {
                    try {
                        const response = await fetch('/api/alerts-incidents/alerts?limit=1');
                        if (response.ok) {
                            const data = await response.json();
