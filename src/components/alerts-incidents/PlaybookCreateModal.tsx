...
interface PlaybookCreateModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (playbook: Partial<InvestigationPlaybook>, classifications: string[]) => Promise<void>;
    availableClassifications: string[];
}

export function PlaybookCreateModal({ 
    isOpen, 
    onClose, 
    onSave, 
    availableClassifications 
}: PlaybookCreateModalProps) {
    ...
    const handleSave = async () => {
        try {
            ...
            await onSave(cleanedData, selectedClassifications);
            onClose();
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g., Advanced Malware Response"
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            {formData.quickResponseGuide.map((step, index) => (
                                <div key={index} className="flex items-center space-x-2">
                                    ...
                                    <input
                                        ...
                                        placeholder={`Step ${index + 1}: ${index === 0 ? 'Verify activity is expected...' : index === 1 ? 'Check for signs of compromise...' : 'If compromise detected: isolate and escalate...'}`}
                                    />
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                    placeholder="When should this alert be escalated to a security incident?"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                    placeholder="When is this alert legitimate business activity?"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                    placeholder="When is this alert a false positive?"
                                />
                            </div>
                        </div>
                    </div>
