...
export async function GET(request: NextRequest) {
  try {
    ...
    // Verify user has Security Analyst role or higher
    ...
    if (userRole !== UserRole.SECURITY_ANALYST && 
      ...
      return NextResponse.json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Only Security Analysts can access security tickets',
        },
      }, { status: 403 });
    }
    ...
    // Get security tickets only
    const _result = await TicketService.getSecurityTickets(
    ...
    const response: ApiResponse = {
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to fetch security tickets',
      },
    };
