/**
 * Demo API endpoint for individual Investigation Playbook details
 * 
 * Provides realistic mock data for playbook details view
 */

import { NextRequest, NextResponse } from 'next/server';
import { InvestigationPlaybook } from '@/types/alerts-incidents';

// Complete mock playbooks data - same as in the main route
...
export async function GET(
    ...
    try {
        ...
        // Mock classification links for this playbook
        const classificationMap: Record<string, { classification: string; isPrimary: boolean }[]> = {
            'playbook-001': [
                { classification: 'malware', isPrimary: true },
                { classification: 'suspicious_file', isPrimary: false }
            ],
            'playbook-002': [
                { classification: 'network_intrusion', isPrimary: true },
                { classification: 'brute_force', isPrimary: false },
                { classification: 'dns_anomaly', isPrimary: false }
            ],
            'playbook-003': [
                { classification: 'phishing', isPrimary: true }
            ],
            'playbook-004': [
                { classification: 'data_loss', isPrimary: true },
                { classification: 'insider_threat', isPrimary: false }
            ],
            'playbook-005': [
                { classification: 'behavioral', isPrimary: true },
                { classification: 'insider_threat', isPrimary: false }
            ],
            'playbook-006': [
                { classification: 'brute_force', isPrimary: true }
            ],
            'playbook-007': [
                { classification: 'privilege_escalation', isPrimary: true }
            ],
            'playbook-008': [
                { classification: 'suspicious_file', isPrimary: true }
            ],
            'playbook-009': [
                { classification: 'dns_anomaly', isPrimary: true }
            ],
            'playbook-010': [
                { classification: 'account_compromise', isPrimary: true }
            ],
            'playbook-011': [
                { classification: 'ransomware', isPrimary: true }
            ],
            'playbook-012': [
                { classification: 'insider_threat', isPrimary: true }
            ]
        };

        const mockClassifications = classificationMap[playbookId] || [];
        ...
        console.error('Demo playbook details API error:', error);
        return NextResponse.json({
            success: false,
            error: {
                code: 'DEMO_ERROR',
...
export async function PUT(
    ...
    try {
        ...
        // Find the existing playbook
        const existingPlaybook = mockPlaybooks.find(p => p.id === playbookId);
        ...
        // In demo mode, simulate updating the playbook
        console.log('Demo: Updated playbook', playbookId, 'with data', playbook, 'and classifications', classifications);

        return NextResponse.json({
            success: true,
            data: {
                message: 'Playbook updated successfully (demo mode)'
            }
        });

    } catch (error) {
        console.error('Demo update playbook error:', error);
        return NextResponse.json({
            success: false,
            error: {
                code: 'DEMO_ERROR',
                message: 'Failed to update playbook in demo mode'
            }
        }, { status: 500 });
    }
}