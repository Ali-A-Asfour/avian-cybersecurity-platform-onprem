...
/**
 * POST /api/alerts-incidents/playbooks/[id]/activate
...
export async function POST(
    ...
    try {
        ...
        // Only Super Admins can activate playbooks
        if (userRole !== 'super_admin') {
            return NextResponse.json(
                { error: 'Insufficient permissions. Only Super Admins can activate playbooks.' },
                { status: 403 }
            );
        }

        await PlaybookManager.activatePlaybook(params.id, userRole);
        ...
        if (error instanceof Error) {
            ...
            if (error.message.includes('already active') || error.message.includes('already has an active')) {
                return NextResponse.json(
                    { error: error.message },
                    { status: 409 }
                );
