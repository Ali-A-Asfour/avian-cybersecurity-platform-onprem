/**
 * Monthly Reports API Endpoint for Alerts & Security Incidents Module
 * 
 * Provides REST API for generating and retrieving monthly reports with:
...
import { MonthlyReportingService, MonthlyReportFilters } from '@/services/alerts-incidents/MonthlyReportingService';
import { ReportCacheService } from '@/services/alerts-incidents/ReportCacheService';
import { logger } from '@/lib/logger';

// Import the monthly report type
...
/**
 * GET /api/alerts-incidents/reports/monthly
 * 
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Validate inputs
        try {
            MonthlyReportingService.validateReportInputs(filters);
        ...
        try {
            report = await ReportCacheService.getCachedReport<MonthlyReport>(cacheParams);
        } catch (cacheError) {
        ...
        if (!report) {
            ...
            report = await MonthlyReportingService.generateMonthlyReport(
                filters,
                user.user_id
            );
            ...
            logger.info('Monthly report generated via API (not cached)', {
                reportId: report.id,
                tenantId: tenant.id,
                userId: user.user_id,
                generationTime,
            ...
            logger.info('Monthly report served from cache', {
                reportId: report.id,
                tenantId: tenant.id,
                userId: user.user_id,
        ...
        logger.error('Failed to generate monthly report via API', error instanceof Error ? error : new Error(String(error)));
...
/**
 * POST /api/alerts-incidents/reports/monthly
 * 
 * Generate monthly report with custom configuration
 * 
 * Request Body:
 * {
 *   "dateRange": {
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Generate report
        const report = await MonthlyReportingService.generateMonthlyReport(
            filters,
            user.user_id
        );
        ...
        if (body.delivery) {
            try {
                ...
                await MonthlyReportingService.deliverMonthlyReport(report, deliveryConfig);
            } catch (deliveryError) {
                logger.error('Failed to deliver monthly report', deliveryError instanceof Error ? deliveryError : new Error(String(deliveryError)), {
                    reportId: report.id,
                    tenantId: tenant.id,
                });
        ...
        logger.info('Monthly report generated and delivered via API', {
            reportId: report.id,
            tenantId: tenant.id,
            userId: user.user_id,
            startDate: startDate.toISOString(),
        ...
        logger.error('Failed to generate monthly report via POST API', error instanceof Error ? error : new Error(String(error)));
